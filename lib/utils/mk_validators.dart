...
class MkValidate {
  ...
  static ValidatorFn<String, String> tryPassword([String error]) {
    return (String value) {
      ...
      if (!value.contains(RegExp(r'^.*(?=.{6,})(?=.*[a-zA-Z0-9]).*$'))) {
  ...
  static ValidatorFn<String, String> tryMeter(String meterNumber,
      [String error]) {
    return (String value) {
      return MkValidate.tryString()(value) ??
          (meterNumber != null && value == meterNumber
              ? null
              : "We couldn't find any registered meters for this number.");
    };
  }
